...
// --- MarketMint (DEPRECATED — kept for reference) ---

// --- MarketMint v2 (Bond-Protected, DEPRECATED) ---

#[test]
#[allow(deprecated)]
fn market_mint_body_size() {
    assert_eq!(MARKET_MINT_BODY.len(), 25, "MarketMint v2 body must be 25 bytes");
}

#[test]
#[allow(deprecated)]
fn market_mint_if_endif_balance() {
    check_if_endif_balance(MARKET_MINT_BODY, "MarketMint v2");
}

#[test]
#[allow(deprecated)]
fn market_mint_redeem_script_size() {
    let admin = [0xaa; 32];
    let market = [0xbb; 32];
    let rs = build_market_mint_redeem_script(&admin, &market, 1_000_000).unwrap();
    // State: [0x20][32B] + [0x20][32B] + [0x08][8B] = 33+33+9 = 75B
    let state_size = 33 + 33 + 9; // 75B
    assert_eq!(state_size, 75);
    assert_eq!(rs.len(), state_size + MARKET_MINT_BODY.len());
    println!("MarketMint v2 RS: {}B state + {}B body = {}B total",
        state_size, MARKET_MINT_BODY.len(), rs.len());
}

#[test]
#[allow(deprecated)]
fn market_mint_validation() {
    let admin = [0xaa; 32];
    let market = [0xbb; 32];
    // expiry_daa must be > 0
    assert!(build_market_mint_redeem_script(&admin, &market, 0).is_err());
    // valid
    assert!(build_market_mint_redeem_script(&admin, &market, 1_000_000).is_ok());
}

#[test]
#[allow(deprecated)]
fn market_mint_mint_sigscript_works() {
    let admin = [0xaa; 32];
    let market = [0xbb; 32];
    let rs = build_market_mint_redeem_script(&admin, &market, 1_000_000).unwrap();
    // Reuse v1 sigscript builders (same format: sig + selector + RS)
    let sig = [0x11; 64];
    let ss = build_market_mint_sigscript(&sig, &rs);
    // Format: [push(sig65B)] [Op1 selector] [pushData(RS)]
    assert_eq!(ss[66], OP_1, "selector must be Op1 (mint)");
}

#[test]
#[allow(deprecated)]
fn market_mint_burn_sigscript_works() {
    let admin = [0xaa; 32];
    let market = [0xbb; 32];
    let rs = build_market_mint_redeem_script(&admin, &market, 1_000_000).unwrap();
    let sig = [0x11; 64];
    let ss = build_market_burn_sigscript(&sig, &rs);
    // Format: [push(sig65B)] [Op0 selector] [pushData(RS)]
    assert_eq!(ss[66], OP_0, "selector must be Op0 (burn)");
}

#[test]
#[allow(deprecated)]
fn market_mint_burn_has_cltv() {
    let body = MARKET_MINT_BODY;
    assert!(body.contains(&OP_CLTV),
        "MarketMint v2 burn path must contain OP_CLTV");
}

#[test]
#[allow(deprecated)]
fn market_mint_mint_has_value_conservation() {
    let body = MARKET_MINT_BODY;
    // Mint path must contain OpTxOutputAmount and OpTxInputAmount
    assert!(body.contains(&OP_TXOUTPUTAMOUNT),
        "MarketMint v2 mint path must check output amount");
    assert!(body.contains(&OP_TXINPUTAMOUNT),
        "MarketMint v2 mint path must check input amount");
}

#[test]
#[allow(deprecated)]
fn market_mint_has_spk_equality() {
    let body = MARKET_MINT_BODY;
    assert!(body.contains(&OP_TXOUTPUTSPK),
        "MarketMint v2 mint path must use OpTxOutputSpk");
    assert!(body.contains(&OP_TXINPUTSPK),
        "MarketMint v2 mint path must use OpTxInputSpk");
}
